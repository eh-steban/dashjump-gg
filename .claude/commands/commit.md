...
**Read `.claude/rules/infra/git.md` before drafting the commit message.** That file is the authoritative source for types, formatting rules, and good/bad examples. Follow it exactly.

1. Run `git status` (never -uall) and `git diff` (staged + unstaged) to understand what changed
2. Read `.claude/rules/infra/git.md` for commit message conventions
